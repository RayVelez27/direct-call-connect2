# Supabase Schema — Plezyy

Foundation tables for the Plezyy marketplace. Run these in order in the Supabase SQL Editor.
...
Extends `auth.users`. One row per sign-up.
...
Detailed profile for creators. 1-to-1 with `profiles`.
...
Identity & KYC data. Kept separate for privacy.
...
Stripe or manual bank details for creators.
...
Individual service offerings listed by a creator.
...
Pricing tiers within a service (Basic / Standard / Premium).
...
A consumer booking a session with a creator.
...
Consumer reviews on completed bookings.
...
Photos, videos, and previews for creator galleries.
...
Direct messages between consumers and creators.
...
Consumers saving creators to their favorites list.
...
## 14. Service Categories (Seed Data)

Reference table for the predefined category list.
...
| # | Table | Purpose |
|---|-------|---------|
| 1 | `profiles` | Base user record (extends auth.users) |
| 2 | `creator_profiles` | Detailed creator info, stats, slug |
| 3 | `creator_verifications` | KYC / identity verification |
| 4 | `payout_accounts` | Stripe or bank payout details |
| 5 | `services` | Service listings by creators |
| 6 | `service_tiers` | Pricing tiers per service |
| 7 | `bookings` | Consumer-to-creator session bookings |
| 8 | `transactions` | Payment records |
| 9 | `reviews` | Ratings & reviews on bookings |
| 10 | `creator_media` | Photos/videos for creator galleries |
| 11 | `messages` | Direct messages |
| 12 | `favorites` | Saved/favorited creators |
| 13 | `service_categories` | Predefined category reference data |
